        ...
        <DialogHeader className="sticky top-0 z-10 border-b border-slate-200 bg-[#F8FBF9] px-6 py-4">
          ...
          <div className="mt-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" className="gap-2" onClick={() => void persistSnapshot().finally(() => downloadReportCsvV2(payload))}>
                <FileSpreadsheet className="h-4 w-4" />
                Tải CSV
              </Button>
              <Button variant="outline" className="gap-2" onClick={() => void persistSnapshot().finally(() => void downloadReportXlsxV2(payload))}>
                <FileSpreadsheet className="h-4 w-4" />
          ...
          <div className="mt-4 grid grid-cols-5 gap-2 rounded-xl bg-[#E8F0EC] p-1">
            {REPORT_TABS_V2.map((tab) => (
              <button
                key={tab.key}
                type="button"
                className={`rounded-lg px-3 py-2 text-sm transition ${activeTab === tab.key ? "bg-white font-semibold text-slate-950 shadow-sm" : "text-slate-600"}`}
          ...
          <div ref={printableRef} className="rounded-2xl bg-white p-5 shadow-sm">
                ...
                <div className="grid gap-4 lg:grid-cols-[1.15fr_0.85fr_1fr]">
                    ...
                    <table className="w-full text-xs">
                      ...
                      <tbody>
                        {payload.breakdownRows.map((row) => (
                          <tr key={`${row.stage}-${row.activity}`} className={row.isDefault ? "bg-red-50 text-red-800" : "border-b"}>
                        ...
                        <tr className="bg-emerald-50 font-bold">
                          <td colSpan={4} className="py-2">Tổng</td>
                  ...
                  <div className="rounded-xl border border-slate-200 p-3">
                    ...
                    <div className="space-y-2 text-sm">
                      <p><span className="text-slate-500">Tên cơ sở:</span> {payload.facility.name}</p>
                      <p><span className="text-slate-500">Mã HS:</span> {payload.sku.cnCode}</p>
                      <p className="font-semibold text-red-700">
                        Cảnh báo: {gapRow ? "CẢNH BÁO ĐỎ: KHUYẾT SCOPE 3" : "Không có red flag"}
                      </p>
                      <p><span className="text-slate-500">Dấu vết:</span> SHA-256 Verified</p>
                      <div>
            ...
            {activeTab !== "overview" && (
              <div className="mt-5 rounded-xl border border-slate-200 p-4">
                <h3 className="mb-3 text-base font-bold">{REPORT_TABS_V2.find((tab) => tab.key === activeTab)?.label}</h3>
                <table className="w-full text-sm">
                  <tbody>
                    {(activeTab === "input" ? payload.evidence.map((item) => ({ field: item.kind, value: item.fileName, source: item.lookupCode, hash: item.sha256 })) :
                      activeTab === "iso14067" ? payload.breakdownRows.map((item) => ({ field: item.stage, value: item.formula, source: item.source, hash: item.kgCo2e.toFixed(4) })) :
                      activeTab === "esgTt01" ? payload.esgRows.map((item) => ({ field: item.scope, value: item.tCO2e, source: item.source, hash: "tCO2e" })) :
                      payload.cbamRows.map((item) => ({ field: item.field, value: item.value, source: "DG TAXUD", hash: item.unit || "" }))).map((row) => (
