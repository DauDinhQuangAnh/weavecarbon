...
export const getDemoAccountPayload = (dataset: DemoDataset) => ({
  ...
  company: {
    ...
    address: (dataset.company as Record<string, unknown>).address ?? null,
    tax_id: (dataset.company as Record<string, unknown>).tax_id ?? null,
    phone: (dataset.company as Record<string, unknown>).phone ?? null,
    created_at: getSeededAt(dataset),
