...
describe("official frontend carbon outputs", () => {
  it("uses one server total and calculation identity across report, export and audit", () => {
    ...
    expect(auditJson.locked).toBe(false);
    ...
    expect(auditJson.carbonAuthority?.calculationId).toBe("22222222-2222-4222-8222-222222222222");
    expect(auditJson.carbonAuthority?.calculationVersion).toBe(7);
    expect(auditJson.carbonAuthority?.engineVersion).toBe("scope-quality-rss-1.0.0");
    expect(auditJson.carbonAuthority?.factorRegistryVersion).toBe("factors-v1:test");
    expect(auditJson.carbonAuthority?.gwpBasis).toBe("IPCC_AR5_100y");
    expect(auditJson.carbonAuthority?.canonicalInputHash).toBe("a".repeat(64));
    expect(auditCsv).not.toContain("demo-preview");
  });
});
